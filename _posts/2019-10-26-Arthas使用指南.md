...
####一、简介 Arthas 是Alibaba开源的Java诊断工具。安装在系统所在服务器上。目前受很多大中小型公司青睐。

#### 二、解决什么问题 & 场景

```
解决问题：
1、查看某个方法的返回值，参数，异常情况时调用等等
2、反编译源码，查看jvm加载的是否为预期的文件内筒
3、以全局视角来查看系统的运行状况、健康状况
4、方法内调用路径及各方法调用耗时
5、查看jvm运行状况
6、加载外部.class文件redefine到jvm里
等等.....

场景：
1）调用接口时，接口返回异常信息，如果该异常信息没有清晰的定位到代码，那么我们通常只能依靠大脑回忆代码，来估计错误发生地了，如果无法估计，一般情况下就会进入测试环境，模拟复现。
2）这个查询，耗时2s，但是该查询内部有穿插调用了其它功能方法，难道手写System.currentTimeMillis()自己做减运算？
以上操作，无法快速定位异常发生具体位置、及耗时详情，浪费时间。
...
[root@crawl arthas]# java -jar arthas-boot.jar 
...
为什么调用一次接口会出现两条记录？
因为Spring初始化时会通过cglib生成ContractSubjectController的代理对象，DispatchServlet真正使用的却是这个代理对象，在代理对象里调用ContractSubjectController的getContractSubjectLogs方法，所以当我们进行tt watch
命令时，会出现两条。
顺序如下：
...
```
这么好用的Arthas，难道只能登陆服务器才能使用？no 它还提供 Web Console。
1）java -jar arthas-boot.jar --target-ip 192.168.168.67
...
2）tunnel server是真的坑。
下载arthas-tunnel-server-3.1.4.jar
直接启动 java -jar arthas-tunnel-server-3.1.4.jar
日志会打印出password

Using generated security password: fb4a402c-aeef-43af-9ce0-fe1f647aeaa7

访问192.168.168.67:8080，输入agentId 进行连接
到这一步时，我怎么也连接不上……

不过WebConsole需要用户在attach成功之后，方可访问
...
```
查找线上问题的利器，就介绍到这里了，具体使用还要参考官方文档，希望Arthas能在大家日后的工作学习中帮助大家！
```
